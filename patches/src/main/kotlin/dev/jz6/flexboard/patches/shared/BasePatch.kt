 ...
 *
 * Merges the extension DEX, seeds the default preferences (both Flexboard's own and Gboard's
 * suggested settings), and adds the Flexboard entry to Gboard's settings screen. Every public
 * patch depends on this, so a user cannot select a feature without the foundation it rests on —
 * the extension is always merged, the defaults are always seeded, and the settings row is always
 * there.
 *
...
internal val basePatch = bytecodePatch(
    ...
    dependsOn(suggestedSettingsPatch)
}
