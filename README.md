...
| [Hidden Features](#hidden-features) | Turns on seven finished Gboard features that a patched build cannot receive: grammar check, on-device proofreading, Emoji Kitchen browse, the custom sticker tab, offline translation, dismissable chips, and search in Gboard's settings. Their flags are delivered per app signature, so resigning the APK means they never arrive and stay off. |  |
