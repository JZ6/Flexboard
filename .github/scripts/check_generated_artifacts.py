...
def check_inventory(problems):
    ...
    # Declared but not published is the *expected* state of a patch between being written and
    # being released: `patches-list.json` is regenerated by CI at release time, and regenerating it
    # locally needs `generatePatchesList`, which needs the Android SDK. Reporting it as a failure
...
def check_readme(problems):
    ...
    for name in sorted(expected):
        if name not in rows:
            problems.append(f"  {name!r} ships but has no README row — a patch users can tick "
                            f"and nothing that tells them what it does")
    for name in sorted(rows - expected):
        ...
        if set(name) <= set("-: ") or "&nbsp;" in name or name.startswith("18."):
            continue
        problems.append(f"  the README table has a row for {name!r}, which no patch declares "
                        f"and no release published")
